<template>
  <v-container fluid class="pa-4 pa-md-8 home-dashboard">
    <div class="mb-6 mb-md-8">
      ...
      <p class="text-medium-emphasis dashboard-section-subtitle">Aquí tienes un resumen de la actividad de tu chatbot
      </p>
    </div>
        ...
        <v-card class="rounded-xl bg-surface dashboard-panel-card" border elevation="0">
          <div class="pa-4 pa-md-5 d-flex justify-space-between align-center border-b ">
            <div>
              ...
              <p class="text-caption text-medium-emphasis mb-0 dashboard-section-subtitle">Últimas interacciones con tu
                chatbot</p>
            </div>
            <v-btn to="/dashboard/conversations" variant="text" size="small" color="primary"
              class="text-none font-weight-bold dashboard-ghost-btn" append-icon="mdi-arrow-top-right">
              Ver todas
            </v-btn>
          </div>
          ...
          <div v-else-if="!conversationPanelData || conversationPanelData.length === 0"
            class="d-flex flex-column justify-center align-center pa-10 text-center dashboard-empty-state">
          ...
          <v-list v-else lines="two" class="pa-0 bg-transparent">
            <template v-for="(chat, i) in conversationPanelData" :key="i">
              <v-list-item class="px-4 px-md-5 py-3 v-card--link dashboard-list-item"
                @click="openConversation(chat.conversationId)">
                ...
                <template v-slot:append>
                  <div class="d-flex flex-column align-end">
                    <span
                      :class="['text-caption font-weight-bold mb-1 d-flex align-center dashboard-status-pill', chat.status === 'Open' ? 'status-open' : 'status-closed']">
